...
def cityreader_stretch(lat1, lon1, lat2, lon2, cities=[]):
  # within will hold the cities that fall within the specified region
    ...
    for city in cities:
        if (float(city.lat) > lat1 and float(city.lat) < lat2 and float(city.lon) > lon1 and float(city.lon) < lon2) or (float(city.lat) > lat2 and float(city.lat) < lat1 and float(city.lon) > lon2 and float(city.lon) < lon1):
            within.append(city)
